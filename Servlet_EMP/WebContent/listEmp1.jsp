...
<%-- 							<%
							...
							<c:forEach var="emp" items="${emps }" varStatus="s">
								<tr>
									...
									<td>${emp.age }</td>
									<td>
										<a href="loademp.do?id=${emp.id }" class="btn btn-primary btn-sm">修改</a>
										<a href="deleteemp.do?id=${emp.id }" onclick="return confirm('是否确定删除${emp.name}');" class="btn btn-danger btn-sm">删除</a>
									</td>
								</tr>
							</c:forEach>					
						</tbody>		
